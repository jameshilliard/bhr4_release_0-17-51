...
void cs_iplip_dump_mibs(cs_iplip_mib_pe_s *pmib)
{
    printk("=============================================\n");
 ...
 * QoS APIs
 */
void cs_iplip_get_mibs(cs_iplip_mib_pe_s *pmib)
{
    pmib = (cs_iplip_mib_pe_s *)CS_IPLIP_MIB_PHY_ADDR;
    
    cs_iplip_dump_mibs(pmib);
    
    return;
} /* cs_iplip_get_mibs() */
EXPORT_SYMBOL(cs_iplip_get_mibs);

void cs_iplip_dump_util()
{
    cs_iplip_mib_pe_s *pmib = (cs_iplip_mib_pe_s *) CS_IPLIP_MIB_PHY_ADDR;

    printk("IPLIP MIBs:: CPU usage             %d%%\n", pmib->iplip_ipc_mib.BusyAccum / ((pmib->iplip_ipc_mib.CurCcount/100)?:1));
}

void cs_iplip_dump_currccount()
{
    cs_iplip_mib_pe_s *pmib = (cs_iplip_mib_pe_s *) CS_IPLIP_MIB_PHY_ADDR;
...
void cs_iplip_clear_mibs(cs_iplip_mib_pe_s *pmib, cs_iplip_mibs_e type)
{
    pmib = (cs_iplip_mib_pe_s *)CS_IPLIP_MIB_PHY_ADDR;
    
    switch(type) {
        case CS_IPLIP_MIBS_ALL:
    	    memset((void *)&pmib->iplip_ni_mib, 0, sizeof(cs_iplip_mib_pe_s));
            break;
 ...
 */
static int cs_proc_iplip_util_read_proc(char *buf, char **start,
		off_t offset, int count, int *eof, void *data)
{
	u32 len = 0;
	cs_iplip_mib_pe_s mib_counter;
 ...
 */
static int cs_proc_iplip_currccount_read_proc(char *buf, char **start,
		off_t offset, int count, int *eof, void *data)
{
	u32 len = 0;
	cs_iplip_mib_pe_s mib_counter;
 ...
 * IPLIP MIBs
 */
static int cs_proc_iplip_mibs_read_proc(char *buf, char **start,
		off_t offset, int count, int *eof, void *data)
{
	u32 len = 0;
	cs_iplip_mib_pe_s mib_counter;
