...
//Bug#40276:		orig_lspid = cs_cb->key_misc.orig_lspid;
		...
		skb->data = temp_data;
#if 0//Bug#40276: Hardware hash didn't create correctly for NAT/WFO.
		if (pe_id == CS_WFO_IPC_PE1_CPU_ID) {
			//cs_cb->key_misc.orig_lspid = ENCAPSULATION_PORT;
			cs_cb->common.ingress_port_id = CS_NI_IRQ_WFO_PE1;
		} else {
			//cs_cb->key_misc.orig_lspid = ENCRYPTION_PORT;
			cs_cb->common.ingress_port_id = CS_NI_IRQ_WFO_PE0;
		}
		cs_cb->key_misc.orig_lspid = orig_lspid;
#endif //Bug#40276
		cs_cb->key_misc.orig_lspid = cs_cb->key_misc.orig_lspid;    //Bug#40276
