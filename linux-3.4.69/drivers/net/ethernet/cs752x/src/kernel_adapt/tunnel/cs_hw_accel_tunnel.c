	...
	for (i = 0; i < CS_IPLIP_TBL_SIZE; i++) {
		if (cs_iplip_tbl.entry[i].valid == 0) {
			...
			cs_iplip_tbl.entry[i].iplip_hdr.l2tph.ver = 
				htons(tunnel->tunnel_cfg.l2tp.ver);
			cs_iplip_tbl.entry[i].iplip_hdr.l2tph.len = 
				htons(tunnel->tunnel_cfg.l2tp.len);
		...
		)
{
	...
	key.l4_valid = 1;
	key.l4_dp = tunnel_entry->tunnel_cfg.l2tp.dest_port;
	key.l4_sp = tunnel_entry->tunnel_cfg.l2tp.src_port;

	/* CS_HM_VID_1_MASK */
	/* VID 0 means it is untagged */
	key.vid_1 = pppoe_port->p_cfg.vlan_tag & VLAN_VID_MASK;
	...
	ret = cs_fe_table_add_entry(FE_TABLE_FVLAN,
 	...
 	if (ret != FE_TABLE_OK) {
		DBG(printk("%s:%d Can't add VLAN result\n",
			__func__, __LINE__));
	...
	while (p) {
		...
		if (p->pppoe_port_id == p_tunnel_cfg->tx_port) {
			...
			*p_tunnel_id = tunnel_node->tunnel_id;
			...
			} else 	if (tunnel_node->dir == 1 /* upstream */) {
				/* create tunnel hash */
		...
		)
{
	...
	printk("%s:%d: dst = 0x%p, src = 0x%p, type = %d\n", __func__, __LINE__, dst, src, type);
	switch (type) {
	case CS_L2TP_TYPE_1:
					   ...
					   unsigned long value)
{
	...
	switch (notify_event) {
		case CS_HAM_ACTION_MODULE_DISABLE:			
			...
			/* disable CORE_VTABLE_TYPE_RE1_IPLIP vtable */
			cs_core_vtable_set_entry_valid(CORE_VTABLE_TYPE_RE1_IPLIP, 0);
			break;
		case CS_HAM_ACTION_MODULE_ENABLE:
			...
			/* enable CORE_VTABLE_TYPE_RE1_IPLIP vtable */
			cs_core_vtable_set_entry_valid(CORE_VTABLE_TYPE_RE1_IPLIP, 1);			
			break;			
