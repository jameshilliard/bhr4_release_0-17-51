 ...
 *return by sec
 */
int __cs_get_ct_lifetime(int protonum)
{
...
#ifdef CONFIG_SYSCTL
		return (*(int *)(nf_conntrack_l4proto_udp4.ctl_table->data)) / HZ;
#else
		return 300;
#endif
	}
...
void (*ptr_death_by_timeout) (unsigned long);
...
static void cs_forward_callback_ct_timeout(unsigned long ul_conntrack)
{
	...
	if (ret != 0) {
		if (ptr_death_by_timeout != NULL) {
			DBG(printk("\t call death_by_timeout\n"));
			/*equal to linux death_by_timeout(ct);*/
			ptr_death_by_timeout((unsigned long)ct);
		}
	} else {
		unsigned long extra_jiffies = __cs_get_ct_lifetime(protonum) * HZ;
		...
		if (time_before(expire_time, jiffies)) {
			...
			if (!nf_ct_is_confirmed(ct)) {
				...
				cs_core_hmu_delete_hash(CS_CORE_HMU_WATCH_SWID64,
						&hmu_value);
				ct->timeout.function = ptr_death_by_timeout;
			} else {
				...
				DBG(printk("\t extend timer extra_jiffies %ld\n",
						newtime - jiffies));
		...
		cs_core_hmu_value_t *value, u32 status)
{
	...
	if (status == CS_CORE_HMU_RET_STATUS_CREATE_SUCCEED) {
		if (ct->timeout.function != NULL) {
			if (ptr_death_by_timeout == NULL) {
				ptr_death_by_timeout = ct->timeout.function;
			} else if (ptr_death_by_timeout != ct->timeout.function) {
				if (ct->timeout.function !=
						cs_forward_callback_ct_timeout) {
					DBG(printk("%s() ERR ptr_death_by_timeout "
						"!= ct->timeout.function \n", __func__));
					return -1;
				} else {
					return 0;
		...
		cs_kernel_accel_cb_t *cs_cb)
{
	...
	if (ct) {
		...
		if ((cs_cb->common.state == TCP_CONNTRACK_FIN_WAIT) ||
		    (cs_cb->common.state == TCP_CONNTRACK_CLOSE_WAIT) ||
		    (cs_cb->common.state == TCP_CONNTRACK_TIME_WAIT) ||
		    (cs_cb->common.state == TCP_CONNTRACK_CLOSE)) {
...
int cs_forward_fill_swid(struct nf_conn *ct, cs_kernel_accel_cb_t *cs_cb)
{
	...
	if (ct) {
		lifetime = __cs_get_ct_lifetime(cs_cb->input.l3_nh.iph.protocol);
