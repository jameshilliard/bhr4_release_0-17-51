				    ...
				    unsigned long count, void *data)
{
	...
	cs_mcast_address_t *entry_org_p = NULL;
	int i;
...
MCS_L2_MCAST_PORT_ADDRESS_CLEAR_INVAL_EXIT:
	printk(KERN_WARNING "Invalid argument\n");
	printk(KERN_WARNING CS_L2_MCAST_PORT_ADDRESS_CLEAR_HELP_MSG, CS_L2_MCAST_PORT_ADDRESS_CLEAR);
	/* if we return error code here, PROC fs may retry up to 3 times. */
	return count;
}
...
MCAST_ADDRESS_ALL_CLEAR_INVAL_EXIT:
	printk(KERN_WARNING "Invalid argument\n");
	printk(KERN_WARNING CS_L2_MCAST_ADDRESS_ALL_CLEAR_HELP_MSG, CS_L2_MCAST_ADDRESS_ALL_CLEAR);
	/* if we return error code here, PROC fs may retry up to 3 times. */
	return count;
}
