 ...
 * cs_core_logic.c
 *
 * $Id: cs_core_mcu.c,v 1.55 2012/10/25 08:56:07 ewang Exp $
 *
...
int cs_core_logic_input_set_cb(struct sk_buff *skb)
{
	...
	DBG(printk("Setting Initialized Control Block!\n"));
	if (skb == NULL)
		return -1;
...
//	cs_cb->common.ingress_port_id = cs_ni_get_port_id(skb->dev);
	...
	DBG(printk("Done setting initial control block.\n"));
	return 0;
		...
		fe_sw_hash_t *key, bool is_from_re)
{
	...
	if ((flag & CS_HM_SPI_VLD_MASK) && (flag & CS_HM_SPI_MASK)) {
		if ((cb_key_info->l3_nh.iph.protocol == IPPROTO_ESP) ||
			...
			key->spi_idx = ntohl(cb_key_info->l4_h.ah_esp.spi);
			key->spi_vld = 1;
		}
	}
		...
		cs_fwd_hash_t *fwd_hash)
{
	...
	if (cb->output_mask & CS_HM_VID_1_MASK) {
		if ((cb->input.raw.vlan_tpid == 0) &&
			...
			result->first_vlan_cmd =
				CS_FE_VLAN_CMD_PUSH_B;
		else if ((cb->input.raw.vlan_tpid != 0) &&
				 (cb->output.raw.vlan_tpid == 0))
			/*REMOVE VLAN*/
			result->first_vlan_cmd =
				CS_FE_VLAN_CMD_POP_A;
		else
			/*REPLACE VLAN*/
			result->first_vlan_cmd =
				CS_FE_VLAN_CMD_SWAP_A;
	...
	if (cb->output_mask & CS_HM_VID_2_MASK) {
		...
		if (result->second_vlan_cmd == CS_FE_VLAN_CMD_PUSH_B) {
			/* if 2nd command is insert,
			 * need to swap frist_cmd and 2nd_cmd
			 */
			i = result->first_vlan_cmd;
			result->first_vlan_cmd = result->second_vlan_cmd;
			result->second_vlan_cmd = i;

			/*
			 * the current second_vlan_cmd is the orignal first_vlan_cmd
			 */
			if (result->second_vlan_cmd == CS_FE_VLAN_CMD_PUSH_B) {
				i = result->first_vid;
				...
				result->second_vlan_cmd = CS_FE_VLAN_CMD_SWAP_D;
			}
	...
	*/
	...
	if (p_sadb->state == SADB_INIT) {
		...
		if ((ret == 0) || (ret == FE_TABLE_EDUPLICATE)) {
			...
			cs_core_hmu_link_src_and_hash(&new_cs_cb, fwd_hash_idx);
		} else {
			*fwd_cnt = 0;
			return -1;
		}
...
#ifdef CONFIG_CS75XX_MTU_CHECK
		if (fwd_app_type == CORE_FWD_APP_TYPE_L3_GENERIC && (cs_cb->common.ingress_port_id == GE_PORT0 || 
...
int cs_core_add_fwd_hash(cs_fwd_hash_t *fwd_hash_entry, u16 *fwd_hash_idx)
{
	...
	ret = cs_core_hmu_add_hash(hash_index, fwd_hash_entry->lifetime);
	if (ret == 0)
...
int cs_core_add_qos_hash(cs_qos_hash_t *qos_hash_entry, u16 *qos_hash_idx)
{
	...
	ret = cs_core_hmu_add_hash(hash_index, 0);
	if (ret == 0) {
		...
		cs_qos_hash_t *qos_hash_entry, u16 *p_qos_hash_idx)
{
	...
	ret = cs_core_hmu_link_fwd_and_qos_hash(fwd_hash_index, qos_hash_index);
	if (ret != 0) {
		cs_core_hmu_delete_hash_by_idx(fwd_hash_index);
...
int cs_core_logic_add_connections(struct sk_buff *skb)
{
	...
	/*check the packet such as 6to4 which we don't support accel now!!*/
	if (cs_cb->input.l3_nh.iph.ver != cs_cb->output.l3_nh.iph.ver)
		return CS_ACCEL_HASH_DONT_CARE;
