			...
			switch (tunnel_iplip_entry->sub_cmd) {
				...
				case CS_IPLIP_TUNNEL_GET:
					...
					if(p_tunnel_cfg->src_addr.afi == CS_IPV4){
						...
						printk("IPV6: ");
						printk("%x:%x:", p_tunnel_cfg->src_addr.ipv6_addr[0], p_tunnel_cfg->src_addr.ipv6_addr[1]);
						printk("%x:%x", p_tunnel_cfg->src_addr.ipv6_addr[2], p_tunnel_cfg->src_addr.ipv6_addr[3]);
					}
					...
					if(p_tunnel_cfg->dest_addr.afi == CS_IPV4){
						printk("IPV4: ");
						printk("0x%x\n", p_tunnel_cfg->dest_addr.ipv4_addr);
					}else{ /* CS_IPV6 */
						printk("IPV6: ");
						printk("%x:%x:", p_tunnel_cfg->dest_addr.ipv6_addr[0], p_tunnel_cfg->dest_addr.ipv6_addr[1]);
						printk("%x:%x\n", p_tunnel_cfg->dest_addr.ipv6_addr[2], p_tunnel_cfg->dest_addr.ipv6_addr[3]);
					}
					/* p_tunnel_cfg->l2tp */
					printk("p_tunnel_cfg->l2tp info:\n Version and friends, Optional Length\n");
					printk("0x%x 0x%x\n", p_tunnel_cfg->l2tp.ver, p_tunnel_cfg->l2tp.len);
					printk("Tunnel ID,   ID of IPv4 hdr\n");
					printk("0x%x 0x%x\n", p_tunnel_cfg->l2tp.tid, p_tunnel_cfg->l2tp.ipv4_id);
					printk("dest_port,   src_port (UDP port of L2TP)\n");
					printk("0x%x 0x%x\n", p_tunnel_cfg->l2tp.dest_port, p_tunnel_cfg->l2tp.src_port);
					break;
				...
				case CS_IPLIP_IPV6_OVER_L2TP_ADD:
					...
					ni_dm_byte((unsigned int)p_ipv6_prefix->ipv6_addr, 16);
				...
				case CS_IPLIP_IPV6_OVER_L2TP_GETNEXT:
					...
					session_id = tunnel_iplip_entry->ipv6_over_l2tp_param.session_id;
					p_ipv6_prefix = &(tunnel_iplip_entry->ipv6_over_l2tp_param.ipv6_prefix);
					if(p_ipv6_prefix->ipv6_addr[0]==0 &&
						p_ipv6_prefix->ipv6_addr[1]==0 &&
						p_ipv6_prefix->ipv6_addr[2]==0 &&
						p_ipv6_prefix->ipv6_addr[3]==0 ){ /* First time: all 0 */
					...
					if(p_ipv6_prefix->afi == CS_IPV4){
						printk("IPV4: ");
						printk("0x%x\n", p_ipv6_prefix->ipv4_addr);
					}else{ /* CS_IPV6 */
						printk("IPV6: ");
						printk("%x:%x:", p_ipv6_prefix->ipv6_addr[0], p_ipv6_prefix->ipv6_addr[1]);
						printk("%x:%x\n", p_ipv6_prefix->ipv6_addr[2], p_ipv6_prefix->ipv6_addr[3]);
					}
					break;
				default:
                        		break;
