...
int ni_mdio_write(int phy_addr, int reg_addr, u16 value)
{
	...
	gpio_cfg.wrd = NI_READL_MODE(PER_GPIO0_CFG);
	gpio_cfg.bf.iopmod |= 0x1; /* Configure GPIO[0] as output */
	NI_WRITEL_MODE(gpio_cfg.wrd, PER_GPIO0_CFG);
	gpio_out.wrd = NI_READL_MODE(PER_GPIO0_OUT);
	...
	NI_WRITEL_MODE(gpio_out.wrd, PER_GPIO0_OUT);
#endif	/* #if 0 */
				   ...
				   int budget, struct napi_struct *napi)
{
	...
	while ((next_link != hw_wr_ptr) && (done < budget)) {
...
#ifdef CONFIG_CS75XX_NI_EXPERIMENTAL_SW_CACHE_MANAGEMENT
		/* Two cache line already invalidated in allocate function
		 * We can skip this invalidation
		 */
		if ((qm_acp_enabled == 0) && (ni_rx_noncache == 0))
		...
		if (unlikely(qm_acp_enabled != 0) && next_link == hw_wr_ptr) {
		...
		if ((instance == CS_NI_INSTANCE_WFO_PE0)
			|| (instance == CS_NI_INSTANCE_WFO_PE1)) {
				...
				if (cs_accel_kernel_module_enable(CS752X_ADAPT_ENABLE_WFO))
					...
					packet_type = CS_APP_PKT_WIRELESS;
#endif
		} else if (((xram_cpu_hdr0.bits.cpuhdr_voq == CPU_PORT7_VOQ_BASE) &&
...
int ni_special_start_xmit_none_bypass_ne(u16 recirc_idx, u32 buf0, int len0, u32 buf1, int len1, struct sk_buff *skb)
{
	...
	wptr = (wptr+1) & (swtxq->total_desc_num-1);
	//smp_wmb();
	NI_WRITEL_MODE(wptr, swtxq->wptr_reg);
	swtxq->wptr = wptr;
	spin_unlock(&swtxq->lock);
...
static void cs_ni_get_voq_map(u16 voq_number)
{
	...
	voq_lkup_data = NI_READL_MODE(NI_TOP_NI_TX_VOQ_LKUP_DATA0) & 0x1F;
...
static void cs_ne_init_cfg(void)
{
	...
	reg_val = NI_READL_MODE(GLOBAL_GLOBAL_CONFIG);
	reg_val |= (0x40000);
	NI_WRITEL_MODE(reg_val, GLOBAL_GLOBAL_CONFIG);
#endif
...
#if 0
	for (i = 0; i <= XRAM_INST_MAX; i++)
		NI_WRITEL_MODE(ni->intr_cpuxram_rxpkt_mask,
				NI_TOP_NI_CPUXRAM_RXPKT_0_INTENABLE_0 + (i * 8));
#endif
#else
...
int moca_set_phylink_status(int phy_link_status)
{
	...
	if (linkstat && (!netif_carrier_ok(pdev))) {
		tp->link_config.link = 1; /* LINK_UP */
		netif_tx_wake_all_queues(pdev);
		netif_carrier_on(pdev);
	} else if ((!linkstat) && netif_carrier_ok(pdev)) {
