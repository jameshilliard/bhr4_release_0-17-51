...
#elif defined(CONFIG_MK_REFERENCEBM)
extern int bcm53xxx_init(void);
#else
...
#if defined(CONFIG_MK_REFERENCEBM)
	/*
...
#ifdef CONFIG_MK_REFERENCEBM
int do_bcm_init(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	int ret, i;
	unsigned int page, reg, val, len;
...
int do_bcm_reg(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	int ret, i;
	unsigned int page, reg, val, len;
