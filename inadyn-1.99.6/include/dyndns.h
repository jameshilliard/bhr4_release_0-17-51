...
#define DYNDNS_GET_MY_IP_HTTP_REQUEST  "http://%s%s"
...
#define CHANGEIP_UPDATE_IP_HTTP_REQUEST_FORMAT \
	"%s://%s%s" \
		"system=dyndns&" \
		"hostname=%s&" \
		"myip=%s "
...
#define EASYDNS_UPDATE_IP_REQUEST_FORMAT	\
	"%s://%s%s" \
	"hostname=%s&"							\
	"myip=%s&"							\
	"wildcard=%s "
...
#define ZONEEDIT_UPDATE_IP_REQUEST_FORMAT	\
	"%s://%s%s" \
	"host=%s&"	\
	"dnsto=%s "

/* tzo doesnt encode password */
#define TZO_UPDATE_IP_REQUEST_FORMAT	\
	"%s://%s%s" \
	"tzoname=%s&"							\
	"ipaddress=%s&"							\
	"system=tzodns&"						\
...
#define SITELUTIONS_UPDATE_IP_HTTP_REQUEST_FORMAT	\
	"%s://%s%s" \
	"id=%s&"							\
	"ip=%s "


#define DNSEXIT_UPDATE_IP_HTTP_REQUEST_FORMAT	\
	"%s://%s%s" \
	"host=%s&"							\
	"myip=%s "
...
#define HE_IPV6TB_UPDATE_IP_REQUEST_FORMAT	\
	"%s://%s%s" \
	"hostname=%s&" \
	"myip=%s "
...
#define DYNDNS_HASH_STRING_MAX_LEN        256     /* chars */
...
typedef int (*ddns_response_ok_func_t) (void *this, int infnr);
...
typedef struct
{
    char str[DYNDNS_HASH_STRING_MAX_LEN];
} ddns_hash_t;
...
typedef struct {
	...
	ddns_server_name_t my_previous_ip_address;
...
typedef struct {
	...
	ddns_hash_t hashes[DYNDNS_MAX_ALIAS_NUMBER];
	int alias_count;
	char protocol[6];
