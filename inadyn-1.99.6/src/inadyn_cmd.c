...
static int set_secure_handler(cmd_data_t *cmd, int num, void *context);
...
static cmd_desc_t cmd_options_table[] = {
	...
	{"-q", 0, {set_secure_handler, NULL}, ""},
	{"--secure",	0,	{set_secure_handler, NULL}, "Use HTTPS"},
...
static int set_secure_handler(cmd_data_t *cmd, int num, void *context)
{
	...
	memcpy(ctx->info[curr_info].protocol, "https", 5);
...
static int get_alias_handler(cmd_data_t *cmd, int num, void *context)
{
	...
	/*hash*/
	hash = strstr(cmd->argv[num],",");
	if (hash)
	{
	    if (sizeof(*ctx->info[curr_info].hashes) < strlen(hash))
			return RC_DYNDNS_BUFFER_TOO_SMALL;

		strcpy(ctx->info[curr_info].hashes[ctx->info[curr_info].alias_count].str, hash);
	    *hash = '\0';
	}
...
static int get_dyndns_system_handler(cmd_data_t *cmd, int num, void *context)
{
	...
	for (curr_info = 0; curr_info < ctx->info_count &&
	     curr_info < DYNDNS_MAX_SERVER_NUMBER && ctx->info[curr_info].system != system; curr_info++) ;
